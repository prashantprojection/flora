...
class _DiagnosisResultViewState extends ConsumerState<DiagnosisResultView> {
  ...
  Future<void> _sendFirstFollowUp() async {
    ...
    if (mounted) {
      Navigator.of(
        context,
      ).push(MaterialPageRoute(builder: (_) => const DiagnosisChatScreen()));
    }
  }
...
class _FeedbackRowState extends State<_FeedbackRow> {
  ...
  @override
  void didUpdateWidget(_FeedbackRow oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (widget.initialFeedback != oldWidget.initialFeedback) {
      setState(() {
        ...
        DiagnosisFeedbackButton(
          ...
          onTap: () {
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text(
                  "Thanks for the feedback! We'll improve Dr. Flo.",
                ),
                duration: Duration(seconds: 2),
              ),
            );
